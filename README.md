...
You may now click the "Start" button at the top to build `MCC Mod Manager.exe`, which will compile the executible into `MCC_Mod_Manager\bin\Debug`. Copy `MCC Mod Manager.exe`, `Blamite.dll`, `Newtonsoft.Json.dll`, and the `Formats/` folder into a new folder of your choosing. All other files will be generated by the app and can be ignored.
